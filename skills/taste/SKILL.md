...
- End when the user can say what they will do and why. Then offer, in one line, to record the judgment and its prediction with `taste-journal`.
...
| Holding one idea | If every experiment succeeded, what would the conclusion say beyond a number going up? Can you state the idea in one sentence without an "and"? Which sub-problem is most likely to fail, and can you try it first in days? Hand off to `conclusion-first` when the user wants the full gate. |
...
| About to write | Who is the reader and what do they believe now? Is the claim something they will reject if stated outright? Hand off to `paper-plan`. |
...
| A new system, dataset, or API appears | Run the fixed list of ways it could fail before deciding whether it is interesting; the list and its procedure are the `threat-list` habit in this repository's `habits/` directory. Finding nothing is a normal result. |
